...
var html2canvas_pro_esm_exports = /* @__PURE__ */ __exportAll({
	Html2CanvasConfig: () => Html2CanvasConfig,
	IMAGE_RENDERING: () => IMAGE_RENDERING,
	PerformanceMonitor: () => PerformanceMonitor,
	Validator: () => Validator,
	createDefaultValidator: () => createDefaultValidator,
	default: () => html2canvas,
...
var TextRenderer = class {
	...
	iterateVerticalGlyphs(text, letterSpacing, baseline, writingMode, renderFn) {
		...
		for (const letter of letters) {
			if (isSidewaysWritingMode(writingMode) || !hasCJKCharacters(letter) && letter.trim().length > 0) {
				...
				this.ctx.rotate(writingMode === 4 ? -Math.PI / 2 : Math.PI / 2);
				renderFn(letter, 0, 0);
				this.ctx.restore();
			} else {
				const savedBaseline = this.ctx.textBaseline;
	...
	*/
	renderTextWithLetterSpacing(text, letterSpacing, baseline, writingMode = 0) {
		if (letterSpacing === 0 && !isVerticalWritingMode(writingMode)) this.ctx.fillText(text.text, text.bounds.left, text.bounds.top + baseline);
			...
			switch (paintOrderLayer) {
				...
				case 1:
					if (styles.webkitTextStrokeWidth && textBound.text.trim().length) {
						...
						if (styles.letterSpacing === 0 && !isVerticalWritingMode(styles.writingMode)) this.ctx.strokeText(textBound.text, textBound.bounds.left, textBound.bounds.top + styles.fontSize.number);
						else this.iterateLettersWithLetterSpacing(textBound, styles.letterSpacing, styles.fontSize.number, styles.writingMode, (letter, x, y) => this.ctx.strokeText(letter, x, y));
				...
				if (lastLine && lastLine.length > 0 && containerBounds) {
					...
					paintOrder.forEach((paintOrderLayer) => {
						switch (paintOrderLayer) {
							case 0:
								this.ctx.fillStyle = asString(styles.color);
								if (styles.letterSpacing === 0 && !isVerticalWritingMode(styles.writingMode)) this.ctx.fillText(truncatedText, firstBound.bounds.left, firstBound.bounds.top + styles.fontSize.number);
								else this.iterateLettersWithLetterSpacing(truncatedBounds, styles.letterSpacing, styles.fontSize.number, styles.writingMode, (letter, x, y) => this.ctx.fillText(letter, x, y));
								break;
							case 1:
								if (styles.webkitTextStrokeWidth && truncatedText.trim().length) {
									...
									this.ctx.lineWidth = styles.webkitTextStrokeWidth;
									this.ctx.lineJoin = typeof window !== "undefined" && !!window.chrome ? "miter" : "round";
									if (styles.letterSpacing === 0 && !isVerticalWritingMode(styles.writingMode)) this.ctx.strokeText(truncatedText, firstBound.bounds.left, firstBound.bounds.top + styles.fontSize.number);
									else this.iterateLettersWithLetterSpacing(truncatedBounds, styles.letterSpacing, styles.fontSize.number, styles.writingMode, (letter, x, y) => this.ctx.strokeText(letter, x, y));
									this.ctx.strokeStyle = "";
									this.ctx.lineWidth = 0;
									this.ctx.lineJoin = "miter";
								}
				...
				switch (paintOrderLayer) {
					...
					case 1:
						if (styles.webkitTextStrokeWidth && text.text.trim().length) {
							...
							if (styles.letterSpacing === 0 && !isVerticalWritingMode(styles.writingMode)) this.ctx.strokeText(text.text, text.bounds.left, text.bounds.top + baseline);
							else this.iterateLettersWithLetterSpacing(text, styles.letterSpacing, baseline, styles.writingMode, (letter, x, y) => this.ctx.strokeText(letter, x, y));
							this.ctx.strokeStyle = "";
							this.ctx.lineWidth = 0;
							this.ctx.lineJoin = "miter";
						}
...
//#endregion
export { html2canvas_pro_esm_exports as t };
